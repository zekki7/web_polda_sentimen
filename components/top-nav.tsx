...
export function TopNav() {
 ...
 const getMenuItems = () => {
  ...
  // No scroll lock needed - sticky nav keeps overlay visible while page scrolls behind it
  ...
  // Detect menu overflow
  useEffect(() => {
    const checkOverflow = () => {
      if (menuContainerRef.current) {
        setIsMenuOverflow(menuContainerRef.current.scrollWidth > menuContainerRef.current.clientWidth)
      }
    }
  ...
  return (
    <>
      <nav className="relative bg-card border-b border-border sticky top-0 z-40 backdrop-blur-sm bg-card/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between md:justify-start h-16 gap-4">
            ...
            <div className="hidden md:flex flex-1 items-center justify-center h-16 min-w-0">
              <div ref={menuContainerRef} className={`flex items-center h-full gap-2 overflow-x-auto scrollbar-hide ${isMenuOverflow ? 'justify-start' : 'justify-center'}`}>
            ...
            {user && (
              <div className="hidden md:block relative flex-shrink-0" ref={profileDropdownRef}>
                {/* Pill */}
                <button
          ...
          {mobileMenuOpen && user && (
            <div className="md:hidden absolute left-0 right-0 top-full bg-card border-b border-border shadow-xl z-40">
